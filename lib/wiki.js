...
$(document).ready(function () {
  ...
  $("#random").on('click', function () {
    $.ajax({
      ...
      success: function success(response) {
        ...
        */
        var pageArray = Object.keys(response.query.pages);
        var url = response.query.pages[pageArray[0]].fullurl;
        window.location = url;
      },
  ...
  $('form').submit(function (e) {
    ...
    var wikiUrl = 'https://en.wikipedia.org/w/api.php?action=query&format=json&prop=info%7Cextracts&generator=search&inprop=url&exchars=200&exlimit=10&exintro=1&gsrsearch=' + term + '&gsrnamespace=0&gsrlimit=10';
    ...
    $.ajax({
      ...
      dataType: 'jsonp',
      success: function success(response) {
        var idArray = Object.keys(response.query.pages);
        var pages = response.query.pages;
        var pageArray = indexSort(pages, idArray);
  ...
  var indexSort = function indexSort(pages, idArray) {
    var pageArray = [];
  ...
  var pageInfoDisplay = function pageInfoDisplay(pages) {
    var url = "";
    var title = "";
    var extract = "";
    var target = "";
